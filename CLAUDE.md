...
## Business model
- Free tier, two stages: 3 anonymous appraisals metered by IP (Redis counter, no signup required — chosen over a cookie because a cookie is trivially reset via incognito/clearing cookies, which an IP counter isn't), then 3 more per account once signed in (server-metered in Postgres, keyed to a signed-in user — same reasoning, survives incognito/different browsers once an account exists)
...
- `app/api/appraise/route.js` — the ONLY place the Anthropic key is used. Proxies to api.anthropic.com, parses the JSON out of Claude's text response. Rate-limited via `lib/rateLimit.js`: a blanket per-IP cap (30/15min) on every call since extraction is open to anonymous callers, plus a per-user cap (20/hr) on metered calls specifically — free users are already bounded by `FREE_LIMIT`, so the per-user cap is what actually bounds spend for Pro, which otherwise has no ceiling. `count: true` calls (the metered analysis step) check `FREE_LIMIT` against an anonymous IP counter (`anonUsageCount`/`incrAnonUsage` in `lib/rateLimit.js`) when there's no signed-in user, or against that user's `freeUsesConsumed` once signed in — 401 once both are exhausted. Signed-in successes create an `Appraisal` row and increment `freeUsesConsumed` in one transaction; anonymous successes just increment the IP counter (no `Appraisal` row — `userId` is required on that model). `count: false` (extraction) calls stay open to anonymous callers unmetered, same as before accounts existed.
...
- `app/api/me/route.js` — replaces the old `/api/usage`. Returns `{ email, pro, remaining, freeLimit, configured }` from `getCurrentUser()`, or, when there's no user, from the anonymous IP counter (`anonUsageCount`) so the nav's "N free left" is accurate before sign-in too.
...
- `lib/rateLimit.js` — sliding-window limiter backed by Upstash Redis (`UPSTASH_REDIS_REST_URL`/`_TOKEN`), shared across every serverless instance. Used by the login route and `/api/appraise`. Started as an in-memory version, which turned out to provide close to no real protection on Vercel — each invocation can land on a different instance with its own memory, confirmed by live testing (32 rapid requests against a 30-request limit all succeeded). One `Ratelimit` instance per distinct `(max, windowMs)` pair, created lazily and cached, since the package wants its window pre-configured rather than passed per call. Also exports `anonUsageCount`/`incrAnonUsage` — a plain persistent Redis counter (not a `Ratelimit` instance, since this needs a running total, not a time-windowed rate) keyed by IP for the anonymous free-tier stage, with a 180-day TTL set on first use just to bound key growth.
...
- Upstash's `slidingWindow` algorithm is an approximation (weighted across two fixed windows), not an exact sliding log — live testing showed 31 requests through a 30-request limit before blocking. Fine for a spend cap; don't rely on it for exact-count guarantees.
