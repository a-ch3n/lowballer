...
- `lib/rateLimit.js` — minimal in-memory sliding-window limiter, per-serverless-instance only. Used by the login route and `/api/appraise`.
...
- `/api/appraise` and `/api/auth/login` are both rate-limited, but `lib/rateLimit.js` is in-memory and per-instance — fine for one Node process, but each serverless instance on Vercel has its own bucket, so the real ceiling under multi-instance traffic is higher than the numbers imply. Swap it for Redis/Upstash before that matters.
